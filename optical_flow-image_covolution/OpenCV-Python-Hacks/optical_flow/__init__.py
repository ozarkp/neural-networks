...
class OpticalFlowCalculator:
    ...
    def __init__(self, frame_width, frame_height, scaledown=1,
        ...
        self.middle_color_bgr = (flow_color_rgb[1], flow_color_rgb[0], flow_color_rgb[2])
    ...
    def processFrame(self, frame, distance=None, timestep=1):
        ...
        if self.prev_gray != None:
            ...
            middle_rect_y = flow.shape[1]/3
            middle_rect_x = flow.shape[0]/3

            middle_rect = flow[middle_rect_y::middle_rect_y*2, middle_rect_x::middle_rect_x*2]
            ...
            for y in range(0, flow.shape[0], self.move_step):
                ...
                for x in range(0, flow.shape[1], self.move_step):
                    ...
                    if x > middle_rect_y and x < middle_rect_y*2 and y > middle_rect_x and y < middle_rect_x*2:
                        middle_xsum += fx
                        middle_ysum += fy
                        cv2.circle(frame2, (x,y), 1, self.middle_color_bgr, -1)

                    cv2.line(frame2, (x,y), (int(x+fx),int(y+fy)), self.mv_color_bgr)

            #Report the movement only if the movement is above a threshold
            if abs(xsum) > 20 or abs(ysum) > 20:

                if (abs(middle_xsum)/middle_rect_x - abs(xsum)/flow.shape[0]) > 0 and (abs(middle_ysum)/middle_rect_y - abs(ysum)/flow.shape[1]) > 0:
